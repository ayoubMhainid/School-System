...
export const searchStudents = async (token,username) =>{
    const response = await axios.get(`${apiUrl}/student/searchStudentsByUsername/${username}`,{
        headers : {
            Authorization : `Bearer ${token}`
        }
    });
...
export const filterStudentsByGender = async (token,gender) =>{
    const response = await axios.get(`${apiUrl}/student/filterStudentsByGender/${gender}`,{
        headers : {
            Authorization : `Bearer ${token}`
        }
    });
