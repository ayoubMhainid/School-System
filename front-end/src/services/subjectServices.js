...
export const getallSubject = async (token) => {
  const response = await axios.get(`${apiUrl}/subject/getSubjects`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
...
export const getSubjectsByteacherAndClass = async (token, classId) => {
  const response = await axios.get(
    `${apiUrl}/subject/getSubjectsByteacherAndClass/${classId}`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
